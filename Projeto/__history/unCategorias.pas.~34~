...
begin
  if MatchText(UpperCase(grCategorias.SelectedField.FieldName), ['ENVIAR', 'INTEGRADO']) then
...
begin
  ...
  begin
    ...
    if DM.qryCategorias.FieldByName(lColuna).AsString = 'S' then
      lValorColunaAtivo := 'N'
    else
      lValorColunaAtivo := 'S';

    DM.qryCategorias.Edit;
    DM.qryCategorias.FieldByName(lColuna).AsString := lValorColunaAtivo;
